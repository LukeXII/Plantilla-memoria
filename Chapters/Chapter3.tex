...
\begin{itemize}
\item Desconectado: la alimentación se encuentra desconectada
\item Conectado: energizado
\item Activo: la salida óptica se encuentra prendida
\end{itemize}

En la \ref{fig:diagFSM} se puede ver un diagrama de transiciones simplificado de la máquina de estados implementada 
...
Con el objetivo de generar una estructura jerárquica por capas, se decidió crear un \textit{wrapper} que se ubique por encima de los drivers de bajo nivel de UART, SPI, ADC y GPIO provistos por la HAL. De esta forma se genera un nivel mas de abstracción que provee un conjunto de funciones o servicios para interactuar con el EDFA, funcionando a modo de puente entre éste y la aplicación de usuario (backend).
...
% no se si es necesaria la tabla. si terminan siendo pocos parametros mejor sacarla y solo mencionarlos
...
Como se mencionó en la sección X, cada bus SPI tiene una interfaz dedicada en el microcontrolador funcionando en modo maestro. La tabla \ref{tab:configSPI} muestra un resumen de la configuración de ambas.
